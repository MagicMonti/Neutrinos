...
class Geodesic(Trajctory):
    def __init__(self, beta, x_0,r_s, r_start):
        ...
        self.u_0 = u_0 = np.array([0, 0,0,beta/r_start], dtype=np.double) 
    ...
    def normalize_u(self,u,x):
        ...
        return np.array([u_t, u_r, u_theta, u_phi], dtype=np.double)
    ...
    def du_dtau(self,u,x):
        ...
        return np.array([-a_0,-a_1,-a_2,-a_3], dtype=np.double)

    def get_path(self, simulation_length=10000, dtau=1000):
        u_0 = self.normalize_u(self.u_0, self.x_0)
        u = np.ones((simulation_length+1,4), dtype=np.double)
        x = np.ones((simulation_length+1,4), dtype=np.double)
...
class Correction(Trajctory):
    ...
    def get_path(self, simulation_length=10000, dtau=1000):
        x = np.ones((simulation_length+1,4), dtype=np.double)
        v = np.ones((simulation_length,4), dtype=np.double)
        x[0] = self.x_0
    ...
    def _up_correction(self,x):
        ...
        return np.array([0, (r - self.r_s)*np.cos(theta)/(4*self.m*r), (-2*r**2*np.sqrt(1 - self.r_s/r)*np.sin(theta) - 2*r**2*np.sqrt(1 - self.r_s/r) + self.r_s)/(8*self.m*r**4), 0], dtype=np.double)
